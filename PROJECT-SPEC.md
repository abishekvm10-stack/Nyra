...
```
This keeps the core idea from the very first architecture discussion (one LLM call, IR-shaped output, adapter layer) — the "adapter" is no longer a per-site DOM script, it's the OS-level clipboard, which is universal by construction. The select/copy/paste steps above are manual by default; an earlier version automated them via a keyboard-simulation library whose simulated keystrokes weren't reliably reaching every app on Windows, so that was removed. **Automatic mode is back** as an opt-in (Windows only, off by default, `automation.js`) — the original failure was traced to a specific timing bug (the hotkey fires while its own modifier is still physically held, corrupting the next synthesized keystroke) rather than the library being unfixable; the new implementation explicitly releases modifiers before/after every synthesized shortcut and ships a self-test harness (`runAutomationSelfTest()` in `main.js`) so a silent failure gets caught immediately instead of discovered later. Manual clipboard-based mode remains the permanent fallback either way.
...
- Manual select → copy → hotkey → compile → paste loop via the OS clipboard (default, all platforms). Optional automatic select/copy/paste (Windows only, opt-in, off by default) — see architecture note above.
